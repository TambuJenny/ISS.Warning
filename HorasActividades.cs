//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace ISS.Warning
{
    using System;
    using System.Collections.Generic;
    
    public partial class HorasActividades
    {
        public string IdHorasActividades { get; set; }
        public string CodHorasActividades { get; set; }
        public string ActividadeId { get; set; }
        public string ProjectoId { get; set; }
        public Nullable<int> HoraDespendida { get; set; }
        public Nullable<int> HoraDespendidaDias { get; set; }
        public Nullable<int> HoraDespendidaSemana { get; set; }
        public string EstadoId { get; set; }
        public Nullable<System.DateTime> DateCriacao { get; set; }
        public Nullable<System.DateTime> DateAtualizacao { get; set; }
    
        public virtual Actividade Actividade { get; set; }
        public virtual Estado Estado { get; set; }
        public virtual Projecto Projecto { get; set; }
    }
}
